...
def featureextraction(rotate_img_list, model):
    ...
    concat_features = base_model_pool_features
    ica = FastICA(n_components=1)
    concat_features = concat_features.transpose()
    final_feature = ica.fit(concat_features).transform(concat_features)
    # final_feature = np.mean(concat_features, axis=0)
...
if __name__ == "__main__":
    ...
    NETWORK = "Resnet_FC"
    PET_CT = 'PET'
    DataDir = '/mnt/d/CROPPED_VOLs/MIP/PET'
    METHOD = 'MAX-ICA'
    csv_filename = f"/home/amirhosein/HECKTOR2022/WHOLEIMAGE_MAMIP/ExtractedFeatures_{METHOD}/Features_{NETWORK}_MA-MIP_WHOLE-IMAGE_{METHOD}.csv"
